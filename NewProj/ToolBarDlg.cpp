...
void CToolBarDlg::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_bToolbar)
	{
		if ((cx - BUTTON_FIXED_SIZE * 20 - 100) < 180) 
			m_Status_Static.ShowWindow(SW_HIDE);
		else
		{
...
void CToolBarDlg::OnAvi() 
{
	...
	if (m_2ndFilePath == "") 
		m_AVIConverterDlg.GetFiles(&m_1stFile, NULL);
	else 
		m_AVIConverterDlg.GetFiles(&m_1stFile, &m_2ndFile);
...
{	
	...
	if (m_1stFile.m_hFile != INVALID_HANDLE_VALUE) 
		m_1stFile.Close();

	if (m_2ndFile.m_hFile != INVALID_HANDLE_VALUE) 
		m_2ndFile.Close();
...
void CToolBarDlg::OnTimer(UINT nIDEvent) 
{
	...
	if (nIDEvent == TIMER_PLAYER1)
	{
		...
		if (!g_bSoundPlay) 
			waveOutSetVolume(g_hWaveOut, (DWORD)((float)m_slider_volume.GetPos() / 100 * 0xFFFF));
		else 
			waveOutSetVolume(g_hWaveOut, (DWORD)((float)m_slider_volume.GetPos() / 100 * 0x0000));
...
//		Init_VideoOut(&CNewProjDlg::m_View1Dlg, &CNewProjDlg:: m_View2Dlg);
	...
	if (CToolBarDlg::g_bView1Turn_ST == true)
	{
		...
		SetDIBitsToDevice(hMemDC1, 0, 0, VIDEOW, VIDEOH, 0, 0, 0, VIDEOH, buf, &bmi1, 0);
	}
...
//View Scale Auto
	...
	{
		if (rect1.Height() * 16 >rect1.Width() * 9) 
		{
			nWidth = rect1.Width(); 
			nHeight = (int)((float)rect1.Width() * 9 / 16); 
			nX = 0; 
			nY = rect1.Height() - (rect1.Height() - nHeight) / 2;
		}
		else 
		{
			nWidth = (int)((float)rect1.Height() * 16 / 9); 
			nHeight = rect1.Height(); 
			nX = (rect1.Width() - nWidth) / 2; nY = rect1.Height();
		}
	}
	else if (CNewProjDlg::g_config_Value_ST.nViewScaleID == 1)
	{
		if (rect1.Height() * 4 >rect1.Width() * 3) 
		{
			nWidth = rect1.Width(); 
			nHeight = (int)((float)rect1.Width() * 3 / 4); 
			nX = 0; 
			nY = rect1.Height() - (rect1.Height() - nHeight) / 2;
		}
		else 
		{
			nWidth = (int)((float)rect1.Height() * 4 / 3); 
			nHeight = rect1.Height(); 
			nX = (rect1.Width() - nWidth) / 2; 
			nY = rect1.Height();
		}
	}
	...
	if (CToolBarDlg::g_bView1Updown_ST == true) 
	{
		nY -= nHeight; 
		nHeight *= -1;
	}
...
void CToolBarDlg::Video2Out(char* buf, int nScaleMode)
{
	...
	if (CToolBarDlg::g_bView2Turn_ST == true)
	{
		...
		SetDIBitsToDevice(hMemDC2, 0, 0, VIDEOW, VIDEOH, 0, 0, 0, VIDEOH, buf, &bmi2, 0);
	}
	...
	if (CNewProjDlg::g_config_Value_ST.nViewScaleID == 0)
	{
		if (rect2.Height() * 16 >rect2.Width() * 9) 
		{
			nWidth = rect2.Width(); 
			nHeight = (int)((float)rect2.Width() * 9 / 16); 
			nX = 0; 
			nY = rect2.Height() - (rect2.Height() - nHeight) / 2;
		}
		else 
		{
			nWidth = (int)((float)rect2.Height() * 16 / 9); 
			nHeight = rect2.Height(); 
			nX = (rect2.Width() - nWidth) / 2; 
			nY = rect2.Height();
		}
	}
	else if (CNewProjDlg::g_config_Value_ST.nViewScaleID == 1)
	{
		if (rect2.Height() * 4 >rect2.Width() * 3) 
		{
			nWidth = rect2.Width(); 
			nHeight = (int)((float)rect2.Width() * 3 / 4); 
			nX = 0; 
			nY = rect2.Height() - (rect2.Height() - nHeight) / 2;
		}
		else 
		{
			nWidth = (int)((float)rect2.Height() * 4 / 3); 
			nHeight = rect2.Height(); 
			nX = (rect2.Width() - nWidth) / 2; 
			nY = rect2.Height();
		}
	...
	if (CToolBarDlg::g_bView2Updown_ST == true) 
	{
		nY -= nHeight; 
		nHeight *= -1;
	}
...
{
	if (CNewProjDlg::g_config_Value_ST.nWndCounts == 0) 
		return;

	if (TimeSeek(false, true, 0, CNewProjDlg::m_nPathArraysPtr))
		*pResult = 0;
}
...
{
	// TODO: Add your control notification handler code here
	if (m_nSpeed == 125 || m_bPlayer1 == false) 
		return;
...
{
	// TODO: Add your control notification handler code here
	if (m_nSpeed == 8000 || m_bPlayer1 == false) 
		return;
...
void CToolBarDlg::OnEndCut() 
{
	...
	if (m_TimeRangeDlg.m_StartDateTime.wMonth < 10) 
		path +="0";
	path += buf;
	itoa(m_TimeRangeDlg.m_StartDateTime.wDay, buf, 10);
	if (m_TimeRangeDlg.m_StartDateTime.wDay < 10) 
		path += "0";
	path += buf;
	path += "-";
	itoa(m_TimeRangeDlg.m_StartDateTime.wHour, buf, 10);
	if (m_TimeRangeDlg.m_StartDateTime.wHour < 10) 
		path += "0";
	path += buf;
	itoa(m_TimeRangeDlg.m_StartDateTime.wMinute, buf, 10);
	if (m_TimeRangeDlg.m_StartDateTime.wMinute < 10) 
		path += "0";
	path += buf;
	itoa(m_TimeRangeDlg.m_StartDateTime.wSecond, buf, 10);
	if (m_TimeRangeDlg.m_StartDateTime.wSecond < 10) 
		path += "0";
	path += buf;
...
UINT AviRecordThreadProc(LPVOID pParam)
{
	...
	if (CToolBarDlg::m_2ndFilePath != "")
		CToolBarDlg::m_2ndSpliter.OpenFile("", 0);
	...
	do
	{
		...
		CToolBarDlg::m_1stSpliter.GetDatas1();
		CToolBarDlg::m_2ndSpliter.GetDatas2();
		if (CToolBarDlg::m_1stSpliter.m_pY == 0) 
			continue;
...
//				memcpy((char*)(m_lpFrame.PushData + 0x7E900), m_2ndSpliter.m_pV2, 0x19500);	
		...
		if (CToolBarDlg::m_1stSpliter.m_file->GetPosition() >= CToolBarDlg::m_1stSpliter.m_file->GetLength() - 0xC80) 
			break;
...
bool CToolBarDlg::TimeSeek(bool bSetPos, bool bPlayEnable, float fSeekPos, int nCurPath)
{
	...
	HINSTANCE hInst = AfxFindResourceHandle(MAKEINTRESOURCE(20170), RT_BITMAP);;
	HBITMAP hBmpPlay = (HBITMAP)::LoadImage(hInst, MAKEINTRESOURCE(IDB_BITMAP_PLAY), IMAGE_BITMAP, 0, 0, 0);
	HBITMAP hBmpPause = (HBITMAP)::LoadImage(hInst, MAKEINTRESOURCE(IDB_BITMAP_PAUSE), IMAGE_BITMAP, 0, 0, 0);

	// mean pause state
	m_btn_Play.SetBitmap(hBmpPlay);
	SetDlgItemText(IDC_PLAY, _T("Play"));
	...
	if (bSetPos) {
		...
		nSeekPos = (fSeekPos * m_Slider_Seek.GetRangeMax());
	}
	else {
		m_Slider_Seek.SetPos(m_Slider_Seek.m_nRealCurPos);
		nSeekPos = m_Slider_Seek.m_nRealCurPos;
	}
	...
	if (m_nThreadCounts == 2)
	{
		...
		// SetTimer(TIMER_SLIDER, 1000, NULL);
		if (m_1stSpliter.Seek(fPos / m_Slider_Seek.GetRangeMax()) == false) 
			return false;

		if (m_2ndSpliter.Seek(fPos / m_Slider_Seek.GetRangeMax(), m_1stSpliter.m_FrameHeader.dwDTS) == false) 
			return false;
...
{
	if (TimeSeek(true, false, fSeekPos, nCurPath))
