...
IStream* CFileObject::GetStream()
{
    if (m_pStream == NULL)
    {
		FuncCreateStreamOnFile CreateStreamOnFile;
		HINSTANCE hInst = LoadLibrary(_T("shlwapi.dll"));
		CreateStreamOnFile = (FuncCreateStreamOnFile)GetProcAddress(hInst, "shlwapi.dll");
		(*CreateStreamOnFile)(m_path, STGM_READ | STGM_SHARE_DENY_NONE | STGM_DELETEONRELEASE, FILE_ATTRIBUTE_NORMAL, FALSE, NULL, &m_pStream);
