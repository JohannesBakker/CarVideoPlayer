...
{
	int  nMax = this->GetRangeMax();
	CRect rt;
	GetClientRect(&rt);
	int nWidth = rt.Width();
	float  nX = (float)point.x;
	m_nRealCurPos = (int)(nX / nWidth * nMax);
	this->SetPos(m_nRealCurPos);
...
void CBitSlider::UpdateToolTips()
{
	...
	m_ctlTips.UpdateTipText(strTipText, this);
	m_ctlTips.UpdateTipText(strTipText, &m_Thumb);
}
