...
CHTMLWriter::CHTMLWriter( IUnknown * pUnk )
{
	...
	if (m_pUnk)
	{
		if (FAILED( m_pUnk->QueryInterface( IID_IWebBrowser2, (void **) &m_pWB ))) m_pWB = NULL;
	}
	else
		m_pWB = NULL;
}
...
CHTMLWriter::~CHTMLWriter( void )
{
	...
	if (m_bReleaseIWebBrowserPtr && m_pWB) m_pWB->Release();
}
